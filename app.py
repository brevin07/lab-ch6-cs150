...
# create new dataframe for missing row
missing_row = pd.DataFrame([{"Year": MIN_YR - 1}])
...
df = pd.concat([df, missing_row], ignore_index=True).sort_values("Year", ignore_index=True).fillna(0)
...
history_df = pd.DataFrame(columns=["cash", "stocks", "bonds", "start_bal", "start_yr", "planning_time"])
...
def make_stacked_bar(slider_input, title):
    ...
    slider_input = [float(value) for value in slider_input]
    for i in range(3):
        fig.add_trace(
            go.Bar(
                x=[slider_input[i]],
    ...
    fig.update_layout(
        ...
        title_x=0.5,
        barmode="group",
        yaxis=dict(title="Asset Allocation"),
...
bonds_allocations = dbc.InputGroup(
    [
        dbc.InputGroupText("Bonds Allocation %"),
        dbc.Input(id="bonds_allocations", disabled=True, className="text-black"),
    ],
    className="mb-3",
)
...
previous_setting_button = dbc.Button(
    "Previous Setting", id="previous_setting_btn", color="secondary", disabled=True, className="mt-3"
)

input_groups = html.Div(
    [start_amount, start_year, number_of_years, end_amount, rate_of_return, bonds_allocations, previous_setting_button],
    className="mt-4 p-4",
)
...
history_table = dash_table.DataTable(
    ...
    columns=[
        ...
        {"name": "Bonds %", "id": "bonds"},
        {"name": "Start Amount ($)", "id": "start_bal"},
        {"name": "Start Year", "id": "start_yr"},
        {"name": "Years", "id": "planning_time"},
    ],
    page_size=10,
    style_table={"overflowX": "scroll"},
)
...
history_card = dbc.Card(
    [
        dbc.CardHeader("History of Used Settings"),
...
def backtest(stocks, cash, start_bal, nper, start_yr):
    ...
    for yr in dff.Year + 1:
        if yr <= end_yr:
            ...
            dff.loc[yr, "Total"] = dff.loc[yr, ["Cash", "Bonds", "Stocks"]].sum().astype(dff["Total"].dtype)
        ...
        dbc.Row(
            dbc.Col(
                [html.H2(
                    ...
                    html.H5("Brevin Tating - CS150",
                className="text-center text-secondary text-black p-2")],
            ),
...
@app.callback(
    ...
    Output("previous_setting_btn", "disabled"),
    Output("cash", "value"),
    ...
    Input("previous_setting_btn", "n_clicks"),
    prevent_initial_call=True,
)
def manage_history_and_stock_slider(stocks, cash, start_bal, planning_time, start_yr, period_number, n_clicks):
    global history_df

    ctx = callback_context
    trigger_id = ctx.triggered[0]["prop_id"].split(".")[0]

    if trigger_id == "previous_setting_btn":
        # Recall previous setting from history
        ...
        previous = history_df.iloc[1].to_dict()
        history_df = history_df.iloc[1:].reset_index(drop=True)

        button_disabled = history_df.shape[0] < 2

        # Ensure stock slider updates correctly
        max_slider = 100 - int(previous["cash"])
        if max_slider > 50:
        ...
        return no_update, no_update, no_update, no_update, no_update, no_update, no_update, start_yr, planning_time, period_number

    else:
        # Save new setting to history
        ...
        return history_df.to_dict("records"), button_disabled, no_update, max_slider, marks_slider, no_update, no_update, no_update, no_update, no_update




@app.callback(
    Output("bonds_allocations", "value"),
    Input("cash", "value"),
    Input("stock_bond", "value"),
)
def update_bonds_allocation(cash, stocks):
    bonds = 100 - (cash + stocks)
    return f"{bonds}%"  # Display Bonds %
...
def update_stacked_bar(stocks, cash):
